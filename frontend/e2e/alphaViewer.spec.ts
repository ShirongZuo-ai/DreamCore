...
test('browser audio starts on demand and comparison mode remains explicit', async ({
  ...
  await expect(page.getByTestId('audio-state')).toContainText('audio enabled');
  await page.getByRole('button', { name: 'Start replay' }).click();
  await expect(page.getByTestId('audio-state')).toContainText('replay playing');
  ...
  await expect(page.getByTestId('audio-state')).toContainText('audio muted');
  await page.getByRole('button', { name: 'Reset sound' }).click();
  await expect(page.getByTestId('audio-state')).toContainText('audio stopped');
});
...
test('SC4001 explains pre-coverage replay and reaches window-end Alpha rows in seconds', async ({
  ...
  await expect(page.getByTestId('real-eeg-window')).toBeVisible();
  await expect(count).toContainText('Reached feature rows: 0');
  ...
  await expect(count).toContainText('Reached feature rows: 2');
...
test('offline replay shows a provenance-explicit simulated intervention prompt', async ({
  ...
  expect(alignment.alphaDisplayEnd).toBeCloseTo(alignment.cursorTime, 2);
