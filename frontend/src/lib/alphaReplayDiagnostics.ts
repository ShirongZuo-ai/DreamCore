...
export type AlphaAnalysisCoverage = {
  ...
  channels: string[];
  productDisplayMinIafConfidence: number | null;
  productDisplayContextSeconds: number;
};
...
export function alphaAnalysisCoverage(
  ...
  if (
    ...
    !(
      raw.product_display_min_iaf_confidence === undefined ||
      (finiteNumber(raw.product_display_min_iaf_confidence) &&
        raw.product_display_min_iaf_confidence >= 0 &&
        raw.product_display_min_iaf_confidence <= 1)
    ) ||
    !finiteNumber(raw.product_display_context_s) ||
    raw.product_display_context_s <= 0 ||
  ...
  return {
    ...
    productDisplayMinIafConfidence:
      raw.product_display_min_iaf_confidence ?? null,
    productDisplayContextSeconds: raw.product_display_context_s,
  };
}

export function productIafIsReliable(
  row: AlphaFeatureRecord | undefined,
  coverage: AlphaAnalysisCoverage | null,
) {
  const minimum = coverage?.productDisplayMinIafConfidence;
  return Boolean(
    row?.iaf_available &&
    row.individual_alpha_frequency_hz !== null &&
    row.iaf_confidence !== null &&
    minimum !== null &&
    minimum !== undefined &&
    row.iaf_confidence >= minimum,
  );
}
...
export function emptyAlphaWindowMessage(
  ...
  if (!coverage) return 'Alpha is not available in this window.';
