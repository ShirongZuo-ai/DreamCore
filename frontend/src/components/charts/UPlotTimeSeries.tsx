...
export type TimeMarker = {
  ...
  label: string;
  color: string;
  provenance: 'derived' | 'manual' | 'simulated' | 'sonification_control';
};
...
export function UPlotTimeSeries({
    ...
    const chart = new uPlot(
      {
        ...
        series: [
          ...
          ...initialLines.map((line) => ({
            label: line.label,
            stroke: line.stroke,
            width: connectPoints ? 1.2 : 0,
            dash: line.dash,
            points: {
              show: showPoints,
              size: showPoints ? 7 : 0,
              width: showPoints ? 2 : 0,
            },
          })),
        ],
      },
  ...
  return (
    <div
      ...
      data-point-rendering={showPoints ? 'observed-glyphs' : 'hidden'}
      data-point-connection={connectPoints ? 'connected' : 'discrete'}
    ...
    >
      ...
      {visibleMarkers.map((marker) => (
        <button
          ...
          className={`absolute bottom-[30px] top-[10px] z-20 w-0.5 ${onMarkerClick ? 'cursor-pointer' : 'pointer-events-none'}`}
        ...
        >
          {showMarkerLabels ? (
            ...
            >
              {marker.label.startsWith('KC')
                ? marker.label
                : marker.provenance === 'simulated'
                  ? 'SIMULATED'
                  : marker.provenance === 'manual'
                    ? 'MANUAL'
                    : 'DERIVED'}
            </span>
          ) : null}
