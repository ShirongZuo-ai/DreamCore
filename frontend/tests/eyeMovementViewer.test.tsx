...
describe('Eye Movement primary viewer integration', () => {
  ...
  it('updates candidates and shared cursor after seek and exposes outside coverage', async () => {
    ...
    await user.click(
      screen.getByRole('button', {
        name: /12.00 s · Eye Movement Candidate/,
      }),
    );
    const selected = screen.getByTestId('selected-eye-event');
    expect(selected).toHaveTextContent('Selected Eye Movement Event');
    expect(selected).toHaveTextContent('12.00 s');
    expect(selected).toHaveTextContent('75.00 µV');
    expect(selected).toHaveTextContent('W');
    expect(screen.getByTestId('eye-movement-panel')).toHaveTextContent(
      'Cursor 13.00 s',
    );
    expect(
      screen.getByText('Advanced current-signal diagnostics'),
    ).toBeVisible();
    expect(screen.getByTestId('eye-activity-chart')).toHaveAttribute(
      'data-display-end-timestamp',
      '13',
  ...
  it('starts real Web Audio only on user action and supports comparison sources', async () => {
    ...
    await user.click(
      screen.getByRole('button', { name: 'Start Sonification' }),
    );
    await waitFor(() =>
